...
class Spider(BaseSpider):
    ...
    # ---------- 智能跳过广告核心方法 ----------
    def _filter_m3u8_ads(self, m3u8_text):
        """
        解析 m3u8 内容，移除包含广告关键词的 TS 片段，
        返回过滤后的 m3u8 文本。若无广告则返回原内容。
        """
        ...
        for line in lines:
            stripped = line.strip()
            # 如果是 TS 分片链接，检查是否包含广告关键词
            if stripped and not stripped.startswith('#'):
                # 可能是 TS URL 或相对路径
                if any(keyword.lower() in stripped.lower() for keyword in self.AD_KEYWORDS):
                    skip_next_ts = True
                    ad_found = True
                    continue  # 跳过这条 TS
                # 也检查上一行的 EXTINF 是否包含广告
                # 但我们已经在上一行处理时会设置 skip 标记
            elif stripped.startswith('#EXTINF'):
                # 检查 EXTINF 中的标题/描述是否包含广告词
                if any(keyword.lower() in stripped.lower() for keyword in self.AD_KEYWORDS):
                    skip_next_ts = True
                    ad_found = True
                    continue  # 不保留这条 EXTINF，连带跳过下一行 TS
                else:
                    # 如果不是广告 EXTINF，重置跳过标记（可能之前没被设置）
                    skip_next_ts = False
            elif stripped.startswith('#EXT-X-DISCONTINUITY'):
                # 广告前后通常有 DISCONTINUITY 标签，可选择性移除
                # 这里简单移除，避免残留，但要注意可能影响正常分段
                # 我们在广告片段后可能连续出现，安全起见先保留，除非确认是广告段
                # 更稳健的做法：若上一个是广告片段，则移除这个 DISCONTINUITY
                pass

            # 如果当前行不是 TS 并且没有因为广告被跳过，则添加
            if not skip_next_ts:
                new_lines.append(line)
            else:
                # 如果 skip_next_ts 为 True，说明刚跳过了 EXTINF，当前行是 TS，也跳过
                # 但是注意顺序：如果上一行是广告 EXTINF，我们已经 continue 了，所以不会走到这里
                # 这里处理的是：上一行是普通 EXTINF 但 TS 本身带广告关键词的情况
                # 我们已经在 TS 检测中设置了 skip_next_ts，但这里需要跳过 TS 后重置标记
                if stripped and not stripped.startswith('#'):
                    # 这是一条被标记跳过的 TS
                    skip_next_ts = False
                    continue
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        # ---------- 过滤广告 ----------
        filtered_m3u8 = self._filter_m3u8_ads(m3u8_text)

        # 如果过滤后内容与原始一致，说明无广告，直接返回原始 URL 以提高速度
